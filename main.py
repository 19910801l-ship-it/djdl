...
class lhc:
    ...
    def get_old_kj_list(self):
        try:
            ...
            for i in range(1,4):
                header = {'content-type':'application/x-www-form-urlencoded','referer':'https://135ka.com/index.php','user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36'}
                r = requests.post('https://135ka.com/index.php/index/getdata2.html',headers=header,data={'lotCode':'lal','ltype':3,'page':i})
                if r.status_code == 200:
                    data = r.json()
                    if data.get('status')==1:
                        ...
                        for item in data.get('data'):
                            temp_dict ={
                                'qs':item.get('kithe'),
                                'tm':item.get('number').split(',')[-1],
                                'sx':sx.get(item.get('sxname')[-1]),
                                'pm':item.get('number').split(','),
                                'px':[sx.get(px) for px in item.get('sxname').split(',')],    
                            }
                            temp_list = []
                            for number in item.get('number').split(','):

...
class lhc:
    ...
    def get_am_q3_qs7_result(self):
        ...
        for index,i in enumerate(new_kj_results_list):
            if i.get('tm') in last3_num_list:
                msg = f'<b>澳门期数7前3颗</b>({self.make_url(new_kj_results_list[-1].get("qs"),0)})\n下期号码:{str(next_number_list) if len(next_number_list)>0 else str(last3_num_list)}\n本期号码:{str(last3_num_list)}\n<s>已开奖,期数:{index+1} ✅</s>\n'
                return msg
        msg = f'<b>澳门期数7前3颗</b>({self.make_url(new_kj_results_list[-1].get("qs"),0)})\n下期号码:{str(next_number_list) if len(next_number_list)>0 else str(last3_num_list)}\n本期号码:{str(last3_num_list)}\n未开奖期数:{len(new_kj_results_list)} ❌\n'
        return msg  
    #澳门8颗
    ...
    def get_am12_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            ...
            if int(kh_result[i]['tm']) in kh12:
                msg = f'<b>澳门12颗</b> ({self.make_url(newqs.get("qs"),0)}) {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n<s>已开奖,期数: {i+1} ✅</s>\n'
                return msg
                break  
        msg = f'<b>澳门12颗</b> ({self.make_url(newqs.get("qs"),0)}){"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {qs_list[qs_index]} ❌\n'
        return msg              

...
class lhc:
    ...
    def kj_show(self):
        ...
        msg += f'【<b>香港开奖</b>】({self.make_url(kj_kh_list.get("qs"),1)})\n'
        ...
        for index,item in enumerate(s3): 
            for i in range(0,40):
                if kj_result[i]['sx'] in item:
                    msg += f'<s>{",".join(item)} 距离开奖期数:{i} { "✅" if i==0 else "❌"}</s>\n' if i==0 else f'{",".join(item)} 距离开奖期数:{i} { "✅" if i==0 else "❌"}\n'
                    break
        return msg
    ...
    def get_amabcd_result(self):
        ...
        msg = f'<b>澳门ABCD</b> ({self.make_url(newqs.get("qs"))})\n'
        for index,item in enumerate(s3): 
            for i in range(0,40):
                if int(kj_result[i]['tm']) in item:
                    msg += f'<s>{zmane[index]} 距离开奖期数: {i} { "✅" if i==0 else "❌"}</s>\n' if i==0 else f'{zmane[index]} 距离开奖期数: {i} { "✅" if i==0 else "❌"}\n'
                    break
        return msg
    ...
    def get_am8_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            if int(kh_result[i]['tm']) in kh12:
                msg = f'<b>澳门8颗</b> ({self.make_url(newqs.get("qs"))}) {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n<s>已开奖,期数: {i+1} ✅</s>\n'
                return msg
                break 
    ...
    def get_am9_result(self):
        ...
        for i in range(0,100):
            if int(kj_result[i]['tm']) in s9:
                msg = f'<b>澳门9颗</b> ({self.make_url(newqs.get("qs"))})\n{str(s9)}\n{f"<s>距离开奖期数: {i} ✅</s>" if i==0 else f"距离开奖期数: {i} ❌"}\n'
                return msg
                break
    ...
    def get_am10_result(self):
        ...
        for i in range(0,100):
            if int(kj_result[i]['tm']) in s10:
                msg = f'<b>澳门10颗</b> ({self.make_url(newqs.get("qs"))})\n{str(s10)}\n{f"<s>距离开奖期数: {i} ✅</s>" if i==0 else f"距离开奖期数: {i} ❌"}\n'
                return msg
                break
    ...
    def get_kh_12_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            if int(kh_result[i]['tm']) in kh12:
                msg = f'<b>香港12颗</b> ({self.make_url(newqs.get("qs"),1)}) {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n<s>已开奖,期数: {i+1} ✅</s>\n'
                return msg
                break  
        msg = f'<b>香港12颗</b> ({self.make_url(newqs.get("qs"),1)}){"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {qs_list[qs_index]} ❌\n'
        return msg  
    ...
    def get_kh_8_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            if int(kh_result[i]['tm']) in kh12:
                msg = f'<b>香港8颗</b> ({self.make_url(newqs.get("qs"),1)}) {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n<s>已开奖,期数: {i+1} ✅</s>\n'
                return msg
                break 
        msg = f'<b>香港8颗</b> ({self.make_url(newqs.get("qs"),1)}) {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {qs_list[qs_index]} ❌\n'
        return msg 
    ...
    def get_kh_2q_qs3_result(self):
        ...
        for index,i in enumerate(new_kj_results_list):
            if int(i.get('tm')) in last3_num_list:
                msg = f'<b>香港期数3平码2</b>({self.make_url(new_kj_results_list[-1].get("qs"),1)})\n下期号码:{str(next_number_list) if len(next_number_list)>0 else str(last3_num_list)}\n本期号码:{str(last3_num_list)}\n<s>已开奖,期数:{index+1} ✅</s>\n'
                return msg
        msg = f'<b>香港期数3平码2</b>({self.make_url(new_kj_results_list[-1].get("qs"),1)})\n下期号码:{str(next_number_list) if len(next_number_list)>0 else str(last3_num_list)}\n本期号码:{str(last3_num_list)}\n未开奖期数:{len(new_kj_results_list)} ❌\n'
        return msg
    ...
    def get_kh_tm_qs3_result(self):
        ...
        for index,i in enumerate(new_kj_results_list):
            if int(i.get('tm')) in last3_num_list:
                msg = f'<b>香港期数3特码</b>({self.make_url(new_kj_results_list[-1].get("qs"),1)})\n下期号码:{str(next_number_list) if len(next_number_list)>0 else str(last3_num_list)}\n本期号码:{str(last3_num_list)}\n<s>已开奖,期数:{index+1} ✅</s>\n'
                return msg
        msg = f'<b>香港期数3特码</b>({self.make_url(new_kj_results_list[-1].get("qs"),1)})\n下期号码:{str(next_number_list) if len(next_number_list)>0 else str(last3_num_list)}\n本期号码:{str(last3_num_list)}\n未开奖期数:{len(new_kj_results_list)} ❌\n'
        return msg
    ...
    def send_msg(self, msg): 
        ...
        r = requests.post(url=f'https://api.telegram.org/bot8389883729:AAHxLMsiLq7tmt-5EW1j-NkEIo_D2AbfVuo/sendMessage', json={'chat_id': '8265835360', 'text': msg,'parse_mode': 'html'})
        
    def make_url(self,qs,type_name=None):
        if type_name==1:
            link = f'https://gaokao.macaujc109.com/guidelines3'
        else:
            link = f'https://macaujc.com'

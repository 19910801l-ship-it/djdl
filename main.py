...
class lhc:
    ...
    def get_am8_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            if int(kh_result[i]['tm']) in kh12:
                msg = f'澳门8颗 {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {i}\n'
                return msg
                break 
    ...
    def get_kh_12_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            if int(kh_result[i]['tm']) in kh12:
                msg = f'香港12颗 {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {i}\n'
                return msg
                break  
        msg = f'香港12颗 {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {qs_list[qs_index]}\n'
        return msg  
    ...
    def get_kh_8_result(self):
        ...
        for i in range(0,qs_list[qs_index]):
            if int(kh_result[i]['tm']) in kh12:
                msg = f'香港8颗 {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {i}\n'
                return msg
                break 
        msg = f'香港8颗 {"1-5期" if qs_index in [1,2,3,4,5] else "6-10期"}\n{str(kh12)}\n未开奖期数: {qs_list[qs_index]}\n'
        return msg 
    ...
    def send_msg(self, msg): 
        bot = '8389883729:AAHxLMsiLq7tmt-5EW1j-NkEIo_D2AbfVuo'
        requests.post(url=f'https://api.telegram.org/bot{bot}/sendMessage', json={'chat_id': '-4823748958', 'text': msg})
